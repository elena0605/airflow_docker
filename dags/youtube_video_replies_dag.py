     ...
     tags=['youtube_video_replies'],
        ...
        def fetch_and_store_video_replies(**context):
            try:
                ...
                context['task_instance'].xcom_push(key='new_reply_ids', value=new_reply_ids)
        ...
        def transform_to_graph(**context):
            try:
                # Get new reply IDs from previous task
                new_reply_ids = context['task_instance'].xcom_pull(
                    task_ids='fetch_and_store_video_replies',
                    key='new_reply_ids'
                )

                if not new_reply_ids:
                    logger.info("No new replies to transform")
                    return

                logger.info(f"Transforming {len(new_reply_ids)} replies to graph")
